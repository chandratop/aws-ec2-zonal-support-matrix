...
def get_regions(all_regions: bool = False) -> list[str]:
    ...
    return [_["RegionName"] for _ in boto3.client('ec2').describe_regions(AllRegions=all_regions)["Regions"]]

def get_instance_types(region: str) -> list[str]:
    """Get a list of available EC2 instance types in a region."""

    return [_["InstanceType"] for page in boto3.client('ec2', region_name=region).get_paginator("describe_instance_types").paginate() for _ in page["InstanceTypes"]]
...
def get_instance_az_availability(instance_type: str, region: str) -> list[str]:
    ...
    return sorted([_["Location"][-1] for _ in boto3.client('ec2', region_name=region).describe_instance_type_offerings(LocationType="availability-zone",Filters=[{"Name": "instance-type", "Values": [instance_type]}])["InstanceTypeOfferings"]])
